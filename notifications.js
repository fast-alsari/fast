...
function emitAppNotice(title, body, type = "info", data = {}) {
  try {
    const detail = { title, body, type, data, message: body || title };
...
async function showBrowserNotification(title, body, data = {}) {
  try {
    ...
    console.log("showBrowserNotification error:", error);
    return false;
  }
}
...
async function sendNotificationToToken(token, title, body, data = {}) {
  if (!NOTIFICATION_WEBHOOK_URL || !token) {
...
async function notifyNewDeliveryOrder(orderData) {
  emitAppNotice("تم إرسال الطلب", "جارٍ البحث عن سائق مناسب", "info", { orderId: orderData?.id || "", type: "delivery" });
...
async function notifyTaxiOrder(orderData) {
  emitAppNotice("تم إرسال طلب التكسي", "جارٍ البحث عن سائق تكسي مناسب", "info", { orderId: orderData?.id || "", type: "taxi" });
...
async function notifyDeliveryStatusChange(order, prevStatus = "") {
  ...
  if (status === "accepted" || status === "driver_accepted") {
    emitAppNotice("تم قبول طلبك", "السائق قبل الطلب وهو الآن يتابع معك", "success", data);
    await Promise.allSettled([
      sendNotificationToToken(
        customerToken,
        "تم قبول طلبك",
...
async function notifyTaxiStatusChange(order, prevStatus = "") {
  ...
  if (status === "accepted" || status === "driver_accepted") {
    emitAppNotice("تم قبول رحلتك", "السائق قبل الرحلة وهو في طريقه إليك", "success", data);
    await Promise.allSettled([
      sendNotificationToToken(
        customerToken,
        "تم قبول رحلتك",
